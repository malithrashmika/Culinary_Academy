...
public class QueryDAOImpl implements QueryDAO {
    ...
    public List<Student> getAllProgramsStudent() {
        ...
        String hqlProgramCount = "select count(*) from Program";
        Query<Long> counthql=session.createQuery(hqlProgramCount, Long.class);
        Long total=counthql.uniqueResult();

        String hql="SELECT Student FROM Student s JOIN S.enrollments e GROUP BY s.studentId HAVING COUNT(DISTINCT e.program.programId) =:totalPrograms";
    ...
    public List<Object[]> getAllEqualByProgramName(String programName) {
        ...
        String hql= "SELECT s,e,p FROM Student s JOIN s.enrollemts e JOIN e.program p WHERE p.programName = :programName";
